...
import { Search, ChevronDown, ChevronLeft, ChevronRight, Edit2, Trash2, Eye } from "lucide-react"
...
export function PassageTable({ passages, onEdit, onDelete, onView }: PassageTableProps) {
  const [searchQuery, setSearchQuery] = useState("")
  const [languageFilter, setLanguageFilter] = useState<"all" | "Filipino" | "English">("all")
  ...
  const filteredPassages = passages.filter((p) => {
    ...
    const matchesLevel = levelFilter === "all" || String(p.level) === levelFilter
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              onClick={() => { setIsLangOpen(!isLangOpen); setIsLevelOpen(false) }}
            ...
            >
              <span>{languageFilter === "all" ? "Language" : languageFilter}</span>
              <ChevronDown className="h-4 w-4" />
            </button>
            {isLangOpen && (
              <div
                className="absolute left-0 top-full z-10 mt-1 w-40 rounded-lg bg-white py-1"
                style={{ boxShadow: "0px 4px 20px rgba(0, 48, 110, 0.15)" }}
              >
                {["all", "Filipino", "English"].map((lang) => (
                  <button
                    key={lang}
                    onClick={() => { setLanguageFilter(lang as typeof languageFilter); setIsLangOpen(false); setCurrentPage(1) }}
                    className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-[#E4F4FF] ${
                      languageFilter === lang ? "font-semibold text-[#6666FF]" : "text-[#00306E]"
                    }`}
                  >
          ...
          <div className="relative">
            <button
              onClick={() => { setIsLevelOpen(!isLevelOpen); setIsLangOpen(false) }}
              className="flex items-center gap-2 rounded-lg border border-[#5D5DFB]/20 bg-white px-4 py-2.5 text-sm font-medium text-[#00306E] transition-colors hover:bg-[#E4F4FF]"
            >
              <span>{levelFilter === "all" ? "Level" : getLevelLabel(Number(levelFilter))}</span>
              <ChevronDown className="h-4 w-4" />
            </button>
            {isLevelOpen && (
              <div
                className="absolute left-0 top-full z-10 mt-1 max-h-60 w-44 overflow-auto rounded-lg bg-white py-1"
                style={{ boxShadow: "0px 4px 20px rgba(0, 48, 110, 0.15)" }}
              >
                <button
                  onClick={() => { setLevelFilter("all"); setIsLevelOpen(false); setCurrentPage(1) }}
                  className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-[#E4F4FF] ${
                    levelFilter === "all" ? "font-semibold text-[#6666FF]" : "text-[#00306E]"
                  }`}
                >
                  All Levels
                </button>
                {levelOptions.map((l) => (
                  <button
                    key={l.value}
                    onClick={() => { setLevelFilter(l.value); setIsLevelOpen(false); setCurrentPage(1) }}
                    className={`w-full px-4 py-2 text-left text-sm transition-colors hover:bg-[#E4F4FF] ${
                      levelFilter === l.value ? "font-semibold text-[#6666FF]" : "text-[#00306E]"
                    }`}
                  >
                    {l.label}
                  </button>
      ...
      >
        {/* Table Header */}
        <div
          className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_80px_80px_100px] px-6 py-3"
          style={{
        ...
        >
          ...
          <span className="text-[15px] font-medium text-[#00306E]">Test Type</span>
          <span className="text-[15px] font-medium text-[#00306E]">Words</span>
          <span className="text-[15px] font-medium text-[#00306E]">{"Q's"}</span>
        ...
        <div className="divide-y divide-[rgba(74,74,252,0.08)]">
          {paginatedPassages.map((passage) => {
            ...
            return (
              ...
              >
                <span className="truncate text-sm font-medium text-[#00306E]">{passage.title}</span>
                <span className="text-sm text-[#00306E]">
                  <span
                    className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                    style={{
                      background: passage.language === "Filipino" ? "rgba(102, 102, 255, 0.12)" : "rgba(84, 164, 255, 0.12)",
                      color: passage.language === "Filipino" ? "#6666FF" : "#54A4FF",
                    }}
                  >
                    {passage.language === "Filipino" ? "FIL" : "ENG"}
                  </span>
                </span>
                <span className="text-sm text-[#00306E]">{getLevelLabel(passage.level)}</span>
                <span className="text-sm">
                  <span
                    className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                    style={{
                      background: tagColors[passage.tags].bg,
                      color: tagColors[passage.tags].color,
                    }}
                  >
                    {passage.tags}
                  </span>
                ...
                <span className="text-sm text-[#00306E]">
                  <span
                    className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                    style={{
                      background: passage.testType === "PRE_TEST" ? "rgba(46, 139, 87, 0.12)" : "rgba(212, 160, 23, 0.12)",
                      color: passage.testType === "PRE_TEST" ? "#2E8B57" : "#D4A017",
                    }}
                  >
                    {getTestTypeLabel(passage.testType)}
                  </span>
                </span>
                <span className="text-sm text-[#00306E]">{passage.wordCount}</span>
                <span className="text-sm text-[#00306E]">{passage.questionsCount}</span>
                <div className="flex items-center justify-end gap-2">
                  <button onClick={() => onView(passage)} className="text-[#54A4FF] transition-opacity hover:opacity-70" title="View">
                    <Eye className="h-4 w-4" />
                  </button>
                  <button onClick={() => onEdit(passage)} className="text-[#162DB0] transition-opacity hover:opacity-70" title="Edit">
                    <Edit2 className="h-4 w-4" />
                  </button>
                  <button onClick={() => onDelete(passage.id)} className="text-[#DE3B40] transition-opacity hover:opacity-70" title="Delete">
