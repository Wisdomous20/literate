...
interface Question {
  ...
  type: string;
  options?: string[];
  correctAnswer?: string; // For MC: correct option; For Essay: key points
  passageId?: string;
}
    ...
    async (e: React.FormEvent) => {
      ...
      if (type === "ESSAY") {
        if (!correctAnswer.trim()) {
          setError("Please provide expected answer/key points for essay questions");
          return;
        }
        if (correctAnswer.trim().length < 10) {
          setError("Expected answer must be at least 10 characters");
          return;
        }
      }
          ...
          {type === "ESSAY" && (
            <div>
              <label
                ...
                className="block mb-1 font-semibold text-[#00306E] text-[15px]"
              >
                Expected Answer / Key Points (Basis for AI Grading)
              </label>
              <textarea
                id="correctAnswer"
                value={correctAnswer}
                ...
                rows={4}
                className="w-full rounded-lg border border-[#E4F4FF] bg-white px-3 py-2 text-sm text-[#00306E] outline-none shadow-sm focus:border-[#6666FF] transition"
                placeholder="Enter the expected answer or key points that the AI should look for when grading this essay question..."
                disabled={isLoading}
              />
              <p className="mt-1 text-xs text-[#00306E]/60">
                This will be used by the AI to grade student responses. Include key concepts, main points, or acceptable answers.
              </p>
            </div>
          )}
        </div>
