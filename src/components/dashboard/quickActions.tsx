...
export function QuickActions({ schoolYear }: QuickActionsProps) {
  ...
  const handleAssessmentClick = (item: RecentAssessmentItem) => {
    // You only have id, assessmentType, studentName, dateTaken, classificationLevel
    // Adjust the path as needed, for example:
    const reportMap: Record<string, string> = {
      ORAL_READING: `/dashboard/report/${item.id}/summary`,
      READING_FLUENCY: `/dashboard/report/${item.id}/reading-fluency-report`,
      COMPREHENSION: `/dashboard/report/${item.id}/comprehension-report`,
    };

    const path = reportMap[item.assessmentType];
    if (path) {
      router.push(path);
    }
  };
 ...
 const pastelBg =
    idx % 4 === 0
      ? "bg-[#F4FBFF]" // softer blue
      : idx % 4 === 1
      ? "bg-[#FFF6FA]" // softer pink
      : idx % 4 === 2
      ? "bg-[#F6FFF7]" // softer green
      : "bg-[#FFFEF6]"; // softer yellow
  return (
    <button
      key={item.id}
