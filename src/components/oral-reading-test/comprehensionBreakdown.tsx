    ...
    >
      ...
      <div className="flex flex-1 flex-col overflow-y-auto">
        {breakdownItems.map((item, index) => {
          ...
          const displayValue = tagData
            ? `${tagData.correct}/${tagData.total}`
            : "--";
      ...
      <div className="mt-auto pt-3">
        <div className="flex flex-col gap-1.5">
          ...
          <div
            className={`flex items-center justify-between rounded px-3 py-1.5 ${levelClasses.bgClass}`}
          >
            <span className="text-xs font-bold text-[#31318A]">
              Comprehension Level:
            </span>
            <span
              className={`font-[Kanit,sans-serif] text-[17px] font-semibold ${levelClasses.textClass}`}
            >
              {level}
            </span>
          </div>
