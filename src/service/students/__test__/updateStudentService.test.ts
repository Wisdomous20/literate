import { beforeEach, describe, expect, it, vi } from "vitest";

const mockPrisma = vi.hoisted(() => ({
  student: { findFirst: vi.fn(), update: vi.fn() },
}));

vi.mock("@/lib/prisma", () => ({ prisma: mockPrisma }));

import { updateStudentService } from "../updateStudentService";

const baseInput = { userId: "user-1", studentId: "student-1", name: "Ana Reyes", level: 3 };
const baseUpdated = { id: "student-1", name: "Ana Reyes", level: 3, classRoomId: "class-1" };

describe("updateStudentService", () => {
  beforeEach(() => vi.clearAllMocks());

  it("returns VALIDATION_ERROR when userId is empty", async () => {
    const result = await updateStudentService({ ...baseInput, userId: "" });

    expect(result.success).toBe(false);
    expect(result.code).toBe("VALIDATION_ERROR");
    expect(mockPrisma.student.findFirst).not.toHaveBeenCalled();
  });

  it("returns VALIDATION_ERROR when studentId is empty", async () => {
    const result = await updateStudentService({ ...baseInput, studentId: "" });

    expect(result.success).toBe(false);
    expect(result.code).toBe("VALIDATION_ERROR");
    expect(mockPrisma.student.findFirst).not.toHaveBeenCalled();
  });

  it("returns VALIDATION_ERROR when name is provided but empty", async () => {
    const result = await updateStudentService({ ...baseInput, name: "   " });

    expect(result.success).toBe(false);
    expect(result.code).toBe("VALIDATION_ERROR");
  });

  it("returns VALIDATION_ERROR when neither name nor level is provided", async () => {
    const result = await updateStudentService({ userId: "user-1", studentId: "student-1" });

    expect(result.success).toBe(false);
    expect(result.code).toBe("VALIDATION_ERROR");
    expect(result.error).toMatch(/Nothing to update/);
  });

  it("returns STUDENT_NOT_FOUND when the student does not belong to the user", async () => {
    mockPrisma.student.findFirst.mockResolvedValue(null);

    const result = await updateStudentService(baseInput);

    expect(result.success).toBe(false);
    expect(result.code).toBe("STUDENT_NOT_FOUND");
    expect(mockPrisma.student.update).not.toHaveBeenCalled();
  });

  it("trims the name before storing", async () => {
    mockPrisma.student.findFirst.mockResolvedValue({ id: "student-1" });
    mockPrisma.student.update.mockResolvedValue(baseUpdated);

    await updateStudentService({ ...baseInput, name: "  Ana Reyes  " });

    const updateCall = mockPrisma.student.update.mock.calls[0][0];
    expect(updateCall.data.name).toBe("Ana Reyes");
  });

  it("updates only level when name is not provided", async () => {
    mockPrisma.student.findFirst.mockResolvedValue({ id: "student-1" });
    mockPrisma.student.update.mockResolvedValue(baseUpdated);

    await updateStudentService({ userId: "user-1", studentId: "student-1", level: 4 });

    const updateCall = mockPrisma.student.update.mock.calls[0][0];
    expect(updateCall.data.level).toBe(4);
    expect(updateCall.data.name).toBeUndefined();
  });

  it("updates only name when level is not provided", async () => {
    mockPrisma.student.findFirst.mockResolvedValue({ id: "student-1" });
    mockPrisma.student.update.mockResolvedValue(baseUpdated);

    await updateStudentService({ userId: "user-1", studentId: "student-1", name: "New Name" });

    const updateCall = mockPrisma.student.update.mock.calls[0][0];
    expect(updateCall.data.name).toBe("New Name");
    expect(updateCall.data.level).toBeUndefined();
  });

  it("returns the updated student on success", async () => {
    mockPrisma.student.findFirst.mockResolvedValue({ id: "student-1" });
    mockPrisma.student.update.mockResolvedValue(baseUpdated);

    const result = await updateStudentService(baseInput);

    expect(result.success).toBe(true);
    expect(result.student).toMatchObject({ id: "student-1" });
  });

  it("scopes the ownership check to the requesting userId", async () => {
    mockPrisma.student.findFirst.mockResolvedValue({ id: "student-1" });
    mockPrisma.student.update.mockResolvedValue(baseUpdated);

    await updateStudentService({ ...baseInput, userId: "user-99" });

    const query = mockPrisma.student.findFirst.mock.calls[0][0];
    expect(query.where).toMatchObject({ id: "student-1", classRoom: { userId: "user-99" } });
  });

  it("returns INTERNAL_ERROR when prisma throws", async () => {
    mockPrisma.student.findFirst.mockRejectedValue(new Error("DB down"));

    const result = await updateStudentService(baseInput);

    expect(result.success).toBe(false);
    expect(result.code).toBe("INTERNAL_ERROR");
  });
});
