...
function makeFormDataRequest(fields: Record<string, string>) {
  ...
  return new NextRequest("http://localhost/api/oral-reading/session", {
    method: "POST",
    body: formData,
  });
}

describe("POST /api/oral-reading/[id]", () => {
  ...
  it("creates a pending ORAL_READING session and enqueues transcription, returns 202", async () => {
    mockCreateAssessmentService.mockResolvedValue({
      success: true,
  ...
  it("passes ORAL_READING type to the assessment service", async () => {
    mockCreateAssessmentService.mockResolvedValue({
      success: true,
      assessment: { id: "assessment-1" },
    });
    ...
    expect(mockCreateAssessmentService).toHaveBeenCalledWith(
      expect.objectContaining({ type: "ORAL_READING" }),
    );
  });
